<template>
  <v-container fluid class="pa-0 ma-0 fill-height">
    <v-sheet width="100%" height="100%" class="d-flex" color="transparent">
      <v-sheet height="100%" class="d-flex flex-grow-1 flex-column justify-start" color="transparent">
        <div class="d-flex mb-4 align-center">
          <v-text-field solo placeholder="Type product name or scan barcode" prepend-inner-icon="mdi-magnify"
            color="primary" class="rounded-lg" :background-color="searchBG" autofocus hide-details
            @focus="changeBG('secondary')" @blur="changeBG('accent')"></v-text-field>
          <v-btn fab tile height="48" width="48" class="ml-4 rounded-lg" elevation="2" color="primary">
            <v-icon>mdi-barcode-scan</v-icon>
          </v-btn>
        </div>
        <v-sheet class="flex-grow-1 d-flex flex-column overflow-hidden" width="100%" height="100%" color="accent"
          ...
          <v-sheet class="flex-grow-1 overflow-y-auto px-2 pt-2" color="transparent" width="100%">
            <v-sheet v-for="(item, index) in cartItems" :key="index" height="56" elevation="1" rounded="lg" class="
                mb-2
                px-3
                d-flex
                align-content-center
                text-sm-subtitle-2 text-caption
                font-weight-medium
              ">
              <v-row no-gutters align="center">
                ...
                <v-col cols="4" lg="5" class="d-flex flex-column justify-start">
                  ...
                  <span v-if="$vuetify.breakpoint.mdAndDown" class="grey--text mt-n1" style="font-size: x-small">{{
                  item.sku }}</span>
                </v-col>
                ...
                <v-col cols="3" lg="2">
                  <v-sheet width="80" max-width="100%" class="d-flex justify-space-between align-center mx-auto">
                    <v-btn tile fab :dark="item.quantity == 1" elevation="1" height="24" width="24" class="rounded"
                      :class="[item.quantity == 1 ? 'red lighten-1' : '']" @click="decreaseQty(item)">
                      <v-icon v-if="item.quantity > 1" small>mdi-minus</v-icon>
                      <v-icon v-else small>mdi-trash-can-outline</v-icon>
                    </v-btn>
                    <span>{{ item.quantity }}</span>
                    <v-btn tile fab elevation="1" height="24" width="24" class="rounded" @click="increaseQty(item)">
        ...
        <v-sheet v-if="$vuetify.breakpoint.mdAndDown" width="100%" elevation="2" class="mt-4" rounded="lg"
          color="accent">
          <v-row no-gutters class="fill-height">
            <v-col cols="12" md="5" class="px-2 pt-2">
              <div class="d-flex align-center">
                <v-combobox v-model="customer" :items="customers" dense outlined
                  class="rounded-lg text-subtitle-2 font-weight-regular" :menu-props="{ bottom: true, offsetY: true }"
                  hide-details></v-combobox>
                <v-btn fab tile height="38" width="38" class="ml-2 rounded-lg" elevation="2">
            ...
            <v-col cols="4" md="2" class="pa-2">
              <v-card @click="paymentModal = !paymentModal" dark ripple width="100%" height="100%" color="primary"
                rounded="lg" elevation="3" class="d-flex flex-column justify-center align-center"
                style="cursor: pointer">
                <v-sheet width="56" height="56" color="primary lighten-1"
